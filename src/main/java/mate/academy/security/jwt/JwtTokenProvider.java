...
public class JwtTokenProvider {
    @Value("${security.spring.jwt.token.secret-key:secret}")
    private String secretKey;
    @Value("${security.spring.jwt.token.expire-length:3600000}")
    ...
    public String createToken(String login, List<String> roles) {
        ...
        Date now = new Date();
        Date validUntil = new Date(now.getTime() + validInMilliseconds);
        return Jwts.builder()
                .setClaims(claims)
                .setIssuedAt(now)
